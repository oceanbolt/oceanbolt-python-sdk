...
from .base import DrydockServiceTransport, DEFAULT_CLIENT_INFO
from .grpc import DrydockServiceGrpcTransport


class DrydockServiceGrpcAsyncIOTransport(DrydockServiceTransport):
    """gRPC AsyncIO backend transport for DrydockService.
    ...
    def get_tonnage_dry_dock(self) -> Callable[
            ...
            Awaitable[service.GetTonnageDryDockResponse]]:
        ...
        Returns:
            Callable[[~.GetTonnageDryDockRequest],
                    Awaitable[~.GetTonnageDryDockResponse]]:
    ...
    def get_dry_dock_stays(self) -> Callable[
            ...
            Awaitable[service.GetDryDockStaysResponse]]:
        ...
        Returns:
            Callable[[~.GetDryDockStaysRequest],
                    Awaitable[~.GetDryDockStaysResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'get_dry_dock_stays' not in self._stubs:
            self._stubs['get_dry_dock_stays'] = self.grpc_channel.unary_unary(
                '/oceanbolt.com.drydock.v3.DrydockService/GetDryDockStays',
                request_serializer=service.GetDryDockStaysRequest.serialize,
...
__all__ = (
    'DrydockServiceGrpcAsyncIOTransport',
)
